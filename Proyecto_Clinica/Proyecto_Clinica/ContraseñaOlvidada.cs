    ...
    public partial class ContraseñaOlvidada : Form
    {
        ...
        private void btn_buscarusuario_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string idkey_usuario = Convert.ToString(txt_correo.Text);
                ...
                resultado = logica.buscarcontraseñaLogica(idkey_usuario);

                if (resultado.Estado)
                {
                    paciente = (Usuarios)resultado.Valor;
                    txt_nombre.Text = paciente.Nombre;
                    txt_idusuario.Text = paciente.ID_Usuario.ToString();
        ...
        private void btn_editarusuario_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                resultado = logica.EditarContraseñaLogica(usuario);
                if (resultado.Estado)
                {
                    MessageBox.Show(resultado.Mensaje);
                    //limpiareditar();
